...
class CarController extends Controller
{
  ...
  {
    return success([
                     'rows' => Car ::wherePartnerId(auth()->guard('partner')->id())->get()
                       -> map(function ($item) {
  ...
  public function store (Request $request): JsonResponse
  {
    ...
    try {
      ...
      $car = $this -> model -> create(array_merge($request -> only([
                                                                     'model_id',
                                                                     'lpn',
                                                                     'color',
                                                                     'year',
  ...
  public function trips($id, Request $request): JsonResponse
  {
    ...
    if ($request->get('from_date')) {
      $query = $query->whereDate('created_at', '>=', Carbon::createFromFormat('Y-m-d-H-i', $request->get('from_date')));
    }

    if ($request->get('from_date')) {
      $query = $query->whereDate('created_at', '=<', Carbon::createFromFormat('Y-m-d-H-i', $request->get('to_date')));
    }

    if ($request->get('cost')) {
      $query = $query->where('cost', '>=', $request->get('cost')[0])->where('cost', '<=', $request->get('cost')[1]);
    }
